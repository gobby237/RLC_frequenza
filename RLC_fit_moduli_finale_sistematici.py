...
# ============================================================
# STIMA SISTEMATICA DA ERRORE DI SCALA RELATIVO SU A
# ============================================================
# Ogni tensione e' misurata con una sonda/canale diverso.
# L'errore standard relativo di scala su ciascuna tensione e':
# sigma_k = 0.4 * 0.03 = 0.012.
# Per il rapporto A = Vout/Vin, gli errori relativi indipendenti
# delle due scale si propagano come sqrt(2) * sigma_k.
sigma_k = 0.4 * scale_error_frac
sigma_A_scale = np.sqrt(2.0) * sigma_k

TR_plus = TR * (1.0 + sigma_A_scale)
TR_minus = TR * (1.0 - sigma_A_scale)
...
res_plus = TR_plus - fit_func(fr, *popt_plus)
res_minus = TR_minus - fit_func(fr, *popt_minus)
chi2_plus = np.sum((res_plus / eTR) ** 2)
chi2_minus = np.sum((res_minus / eTR) ** 2)
chi2rid_plus = chi2_plus / df
chi2rid_minus = chi2_minus / df

# Errore sistematico: semidispersione tra i due fit con scala variata.
perr_syst = np.abs(popt_plus - popt_minus) / 2.0
perr_tot = np.sqrt(perr ** 2 + perr_syst ** 2)

# Grandezze derivate utili: f0 = omega0/(2*pi*1e3) in kHz.
f0_nom = popt[1] / (2.0 * np.pi * 1e3)
f0_plus = popt_plus[1] / (2.0 * np.pi * 1e3)
f0_minus = popt_minus[1] / (2.0 * np.pi * 1e3)
ef0_stat = perr[1] / (2.0 * np.pi * 1e3)
ef0_syst = abs(f0_plus - f0_minus) / 2.0
ef0_tot = np.sqrt(ef0_stat ** 2 + ef0_syst ** 2)

print('========= FIT CON SCALA A VARIATA PER SISTEMATICA =====')
print(f'sigma_k singola tensione = {sigma_k:.6g}')
print(f'sigma_A/A = sqrt(2)*sigma_k = {sigma_A_scale:.6g}')
print('')
print('Fit con A_i -> A_i * (1 + sigma_A/A):')
print(f'A      = {popt_plus[0]:.6g}')
print(f'omega0 = {popt_plus[1]:.6g} rad/s   = {popt_plus[1] * 1e-3:.6g} 10^3 rad/s')
print(f'f0     = {f0_plus:.6g} kHz')
print(f'Q      = {popt_plus[2]:.6g}')
print(f'chi2/ndf = {chi2rid_plus:.6g}  (chi2 = {chi2_plus:.6g}, ndf = {df})')
print('')
print('Fit con A_i -> A_i * (1 - sigma_A/A):')
print(f'A      = {popt_minus[0]:.6g}')
print(f'omega0 = {popt_minus[1]:.6g} rad/s   = {popt_minus[1] * 1e-3:.6g} 10^3 rad/s')
print(f'f0     = {f0_minus:.6g} kHz')
print(f'Q      = {popt_minus[2]:.6g}')
print(f'chi2/ndf = {chi2rid_minus:.6g}  (chi2 = {chi2_minus:.6g}, ndf = {df})')
print('=======================================================')

print('===== STIME FINALI: STATISTICO SCIPY + SISTEMATICO ====')
print('Errore sistematico = semidispersione dei due fit con scala variata.')
print('Errore totale = sqrt(errore_SciPy^2 + errore_sistematico^2).')
print('')
print('Parametro        valore nominale        sigma_SciPy        sigma_sist        sigma_tot')
print(f'A        {popt[0]:16.8g}  {perr[0]:16.4g}  {perr_syst[0]:16.4g}  {perr_tot[0]:16.4g}')
print(f'omega0   {popt[1]:16.8g}  {perr[1]:16.4g}  {perr_syst[1]:16.4g}  {perr_tot[1]:16.4g}   rad/s')
print(f'B_eq     {popt[1] * 1e-3:16.8g}  {perr[1] * 1e-3:16.4g}  {perr_syst[1] * 1e-3:16.4g}  {perr_tot[1] * 1e-3:16.4g}   10^3 rad/s')
print(f'f0       {f0_nom:16.8g}  {ef0_stat:16.4g}  {ef0_syst:16.4g}  {ef0_tot:16.4g}   kHz')
print(f'Q        {popt[2]:16.8g}  {perr[2]:16.4g}  {perr_syst[2]:16.4g}  {perr_tot[2]:16.4g}')
print('=======================================================')
